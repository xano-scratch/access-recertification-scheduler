...
/**
 * Recertify: mark the review signed_off, stamp the entitlement's
 * last_recertified_at, and append an audit row. Reviewer role or above. A review
 * that was already actioned cannot be actioned again (a state guard, HTTP 400).
 */
export const signoffQuery = query({
  name: "reviews/{review_id}/signoff",
  verb: "POST",
  apiGroup: recertApi,
  auth: users,
  ...
  stack: [
    ...
    s.db.edit({ table: entitlements, fieldName: "id", fieldValue: ref("rev.entitlement_id"), row: { last_recertified_at: c.now() } }),
    s.db.edit({ table: recertReviews, fieldName: "id", fieldValue: inp("review_id"), row: { status: "signed_off" }, as: "updated" }),
    s.db.add({
      ...
      row: {
        ...
        reviewer_id: auth("id"),
        decision: "signed_off",
        note: withFilters(inp("note"), fl.first_notempty("Recertified")),
        acted_at: c.now(),
